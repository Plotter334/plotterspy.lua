...
newButton("Loop it up!", function()
    return "This button will copy the trigger with a loop effect."
end, function()
    local triggerCode = codebox:getString()
    local indentedCode = ""
    for line in triggerCode:gmatch("[^\r\n]+") do
        indentedCode = indentedCode .. "   " .. line .. "\n"
    end
    local loopedCode = "while true do\n" .. indentedCode .. "   wait(1)\nend"
    setclipboard(loopedCode)
    TextLabel.Text = "Copied and looped successfully!"
end)

newButton("Preview Loop", function()
    return "This button will show a preview of the looped code."
end, function()
    local triggerCode = codebox:getString() -- Get the code from the code box

    -- Add the loop effect with trigger and wait(1) for preview
    local indentedCode = ""
    for line in triggerCode:gmatch("[^\r\n]+") do
        indentedCode = indentedCode .. "   " .. line .. "\n"
    end

    local previewCode = "while true do\n    " .. indentedCode .. "    wait(1)--in seconds 0.1 is thefastest dont delete if you will try to your game will crash\nend"

    codebox:setRaw(previewCode) -- Display the previewed looped code in the code box
    TextLabel.Text = "Previewed loop successfully!"
end)
